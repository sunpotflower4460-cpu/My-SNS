...
describe('GET /api/messaging/run — CRON_SECRET gate', () => {
  ...
  it('fails closed with 503 when CRON_SECRET is unset', async () => {
    const response = await GET(new NextRequest('http://localhost/api/messaging/run'))
  ...
  it('rejects with 401 when the Authorization header does not match CRON_SECRET', async () => {
    ...
    const response = await GET(
      new NextRequest('http://localhost/api/messaging/run', {
