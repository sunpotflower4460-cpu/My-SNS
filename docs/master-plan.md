...
- `social_drafts`と`draft_revisions`に`ai_original_snapshot`列（JSONB、`{title, body, hashtags, cta}`）を追加。AI提案（`source='ai'`）は生成直後に in-memory で凍結し、初回INSERTでそのコピーを保存する。以降の編集では上書きしない。保存前に直しても「AI original」側へ混入しない。生成時コピーが渡されなかった場合のみ、従来どおり初回保存内容へフォールバックする。`approve_social_draft()`もこの列を`draft_revisions`へコピーする（CREATE OR REPLACE、SECURITY INVOKERのまま変更なし）。
- 新しい`/app/analytics`ページ（`view_analytics`権限、全ロールに付与——閲覧専用のため）。`publish_attempts`・`ai_generations`・`draft_revisions`をワークスペース全体で読み込み（`AppProvider`に追加）、以下を実データのみで表示: 媒体別の公開成功率、失敗理由の内訳、AIコスト・トークン使用量、AI提案が承認前に人間に編集された割合、直近の「AI提案 → 承認した差分」。推定値やチャートライブラリの追加は行わず、既存のstat card/tableスタイルを踏襲（master-plan §7「派手な分析より発信フロー優先」を遵守）。
...
- AI学習: `/api/drafts/generate`が生成前に`listRecentAiRevisionsForStyleLearning()`（チャンネルごと直近3件、AI提案から実際に編集されたRevisionのみ）を取得し、本文だけでなく title / hashtags / CTA の差分を`DraftGenerationContext.styleExamples`としてAnthropicプロンプトへfew-shot例として渡す。同一媒体で2件以上の修正がある場合のみ、事実を含まない傾向メモ（短くする・ハッシュタグを減らす等）を`styleTendencies`として添える。Brand Profileは自動では上書きしない。学習データの取得失敗時は生成自体を止める（後続の信頼性ハードニングで fail-closed に揃えた）。
