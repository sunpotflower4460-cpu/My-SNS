-- Make a human-recorded publish completion one database transaction.
-- The previous browser flow inserted a successful publish_attempt first and
-- then updated publish_jobs to published in a separate request. A transient
-- failure between those writes could leave analytics saying "success" while
-- the queue still said pending/failed.
...
BEGIN
  ...
  -- Lock the job row so two completion clicks cannot allocate the same attempt
  -- number or race each other into conflicting final states.
  SELECT *
  INTO v_job
  ...
  -- Idempotent retry: if this user's earlier request committed but the HTTP
  -- response was lost, return that successful attempt rather than creating a
  ...
  -- API-first auto jobs remain protected by default. The application passes
  -- p_allow_auto=true only in explicit zero-cost mode to reconcile legacy jobs
  -- that predate the current manual scheduling strategy.
...
COMMENT ON FUNCTION public.complete_manual_publish(UUID, UUID, TEXT, TEXT, BOOLEAN) IS
  'Atomically records a human-confirmed successful publish attempt and marks its publish job published. Owner/admin only; idempotent for a retried committed completion.';
