...
- 実用化パス（2026-09）— 実装済み。ローカルSupabase＋メール/パスワードログイン、ワークスペース初回作成、コントラスト改善に加え、設定画面（`#connections`）で媒体ごとの接続可否を`/api/social/status`から取得し、未設定なら不足の環境変数・登録すべきRedirect URI・開発者コンソールへのリンク（`platform-setup.ts`）を表示する。暗号鍵未設定のままOAuthへ進む行き止まりは接続開始前に止める。手動投稿モード（既定）では接続は任意である旨を明記。
