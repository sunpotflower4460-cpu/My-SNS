...
- 日本語UI対応 — マージ済み。フレームワーク（`next-intl`等）は導入せず、全ページ・共有コンポーネント・エラーメッセージ・通知文言を直接日本語に置き換え（唯一の実ユーザーが日本語話者のため）。ブランド名（YouTube/Instagram/X/TikTok/note等）と「Seed」はそのまま維持。翻訳に合わせて`uppercase`＋広いletter-spacingの英字バッジ装飾を除去・縮小（日本語には合わないため）。ついでに見つかったUI/UXの実バグも修正: `PlatformBadge`の`uppercase`が"YouTube"/"TikTok"を"YOUTUBE"/"TIKTOK"に破壊していた点、Settings画面が`capitalize`CSSで媒体名を表示しプラットフォーム名の正しい大文字小文字を壊していた点（`PUBLISHING_CHANNEL_CONFIG`のlabelを使うよう修正）。
